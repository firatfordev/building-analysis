...
import {
  ...
  ExternalLink, CheckCircle, Clock, Edit2,
} from 'lucide-react';
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!res.ok) {
          setLoadError(json.error ?? 'Failed to load building data.');
          return;
        }
        setBuilding(json.building);
      } catch {
        setLoadError('Network error – could not load building data.');
      } finally {
        setLoading(false);
      }
    };
    ...
    <main className="min-h-screen bg-slate-50 pb-16">
      ...
      <header className="bg-[#0A0F1C] border-b border-white/[0.05] px-8 py-4 sticky top-0 z-50 shadow-xl shadow-black/20">
        <div className="max-w-[1440px] mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              href={`/${locale}/admin/buildings/${id}/edit`}
              className="flex items-center gap-2 bg-white/[0.06] border border-white/[0.1] text-slate-300 hover:text-white hover:bg-white/[0.1] transition-all text-[9px] font-bold uppercase tracking-[0.2em] px-4 py-2 rounded-xl"
            >
              <Edit2 className="w-3 h-3" /> Edit Building
            </Link>
            {hasReport && (
              <a
        ...
        {hasReport ? (
          ...
          <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm">
            <div className="flex flex-col items-center justify-center py-28 text-center px-8">
              ...
              <h3 className="text-lg font-bold text-slate-900 mb-2">No Report Uploaded Yet</h3>
              <p className="text-slate-400 text-sm font-light max-w-sm mb-8 leading-relaxed">
                The analysis report PDF has not been uploaded for this building. Edit the record to attach a report.
              </p>
              <Link
                href={`/${locale}/admin/buildings/${id}/edit`}
                className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-[10px] font-bold uppercase tracking-[0.2em] px-6 py-3.5 rounded-xl hover:from-blue-500 hover:to-indigo-500 transition-all shadow-[0_4px_14px_rgba(37,99,235,0.22)]"
              >
                <Edit2 className="w-3.5 h-3.5" /> Upload Report PDF
              </Link>
            </div>
          </div>
        )}
